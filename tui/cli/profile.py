...
def new_profile(
    ...
    if not re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_-]*$", name):
        raise typer.BadParameter(
            "name must start with alphanumeric and contain only [A-Za-z0-9_-]",
            param_hint="NAME",
        )
...
def quick_setup(
    model: str = typer.Argument(..., help="HF model id, e.g. Qwen/Qwen3-8B."),
    backend: str = typer.Option(
        "vllm", "--backend", "-b", help=f"Backend ({', '.join(BACKENDS)})."
    ),
    name: str = typer.Option(
        "", "--name", "-n", help="Profile name (auto-derived from model if omitted)."
    ),
    ...
    copy_config_from: str = typer.Option(
        "", "--copy-from",
        help="vLLM only: copy extra_params from an existing config name.",
    ),
) -> None:
    """Create a profile + config from a model id in one step (mirrors TUI Quick Setup)."""
    if backend != "vllm":
        # llama.cpp quick-setup needs hf_repo/hf_file, not yet covered here.
        raise typer.BadParameter(
            "quick-setup currently only supports backend=vllm; "
            "use `profile new --backend llamacpp` directly for llama.cpp profiles.",
            param_hint="--backend",
        )
