...
여러 GPU가 있는 서버에서 다양한 LLM/VLM 모델을 프로필 기반으로 손쉽게 관리하고, GPU 별 포트 매핑을 자동화하며, vLLM 소스 빌드까지 지원합니다.

### 💡 왜 만들었나요?

여러 GPU가 있는 서버에서 vLLM으로 모델 서빙을 하다 보면...

- **문제:** 이 모델 테스트하려고 올렸다가, 저 모델로 바꾸려고 내렸다가, 다시 다른 모델 올렸다가... GPU 번호 뭐였지? 포트 뭐였지?
- **해결:** 프로필 파일 하나 만들어두면 `./run.sh vlm up` 한 줄로 끝.

**너무 귀찮아서** 만들었습니다.

### 🎯 대상 사용자

- **AI/ML 개발자**: 여러 LLM/VLM 모델을 빠르게 테스트하고 비교해야 하는 개발자
- **AI 연구자**: 다양한 모델 구성과 LoRA 어댑터를 실험하는 연구자
- **DevOps/인프라 엔지니어**: GPU 서버에서 모델 서빙 환경을 효율적으로 관리해야 하는 엔지니어
...
vLLM Compose를 사용하면 기존 방식보다 얼마나 편리할까요?

| 특징 | **vLLM Compose** | 직접 Docker 실행 | Kubernetes |
|:---:|:---:|:---:|:---:|
| **설정 관리** | 프로필 파일 (.env) | 긴 docker run 명령 | 복잡한 YAML |
| **모델 전환** | `./run.sh {profile} up` | 컨테이너 재생성 필요 | Pod 재배포 |
| **GPU 관리** | 자동 감지 및 매핑 | 수동으로 device 지정 | NodeSelector 설정 |
| **포트 관리** | 프로필별 자동 할당 | 수동으로 -p 옵션 | Service 설정 필요 |
| **LoRA 어댑터** | 경로 자동 매핑 | Volume mount 수동 설정 | PV/PVC 필요 |
| **소스 빌드** | Fast Build 지원 | Dockerfile 직접 작성 | CI/CD 파이프라인 |
| **학습 곡선** | ⭐ 낮음 | ⭐⭐ 중간 | ⭐⭐⭐ 높음 |
| **적합한 환경** | 개발/연구용 GPU 서버 | 간단한 단일 모델 서빙 | 프로덕션 클러스터 |
...
│   ├── vlm.env           # Vision-Language 모델 프로필
│   ├── llm.env           # 대규모 언어 모델 프로필
│   └── clova.env         # CLOVA 모델 프로필
├── config/                # vLLM 설정 (YAML 파일)
│   ├── qwen3-vl-30b-a3b-fp8.yaml    # Qwen3-VL 모델 설정
│   └── gpt-oss-120b.yaml             # GPT 모델 설정
